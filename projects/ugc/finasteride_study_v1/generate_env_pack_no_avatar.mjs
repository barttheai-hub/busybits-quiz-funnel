#!/usr/bin/env node
/*
  Generate 3 environment anchors WITHOUT avatar (modern well-lit house).
  Outputs:
    ~/Desktop/ugc-pipeline/projects/ugc/finasteride_study_v1/assets/tmp/env_pack_no_avatar/
*/
...
async function main(){
  ...
  const outDir=path.join(projRoot,'assets/tmp/env_pack_no_avatar');
  ...
  const global = [
    ...
    'NO PEOPLE in the image. No watermark, no UI overlays, no phone screen frame, no text.'
  ].join(' ');

  const env1 = `${global} Environment 1: modern living room / home office corner with clean desk, minimalist bookshelf, large window daylight, plants, neutral tones.`;
  const env2 = `${global} Environment 2: modern bathroom with clean sink, mirror, subtle realistic clutter (toothbrush, soap), warm-white lighting.`;
  ...
  const m=[];
  m.push(await nb(env1, path.join(outDir,'env_01_modern_office_no_avatar.png')));
  m.push(await nb(env2, path.join(outDir,'env_02_modern_bathroom_no_avatar.png')));
  m.push(await nb(env3, path.join(outDir,'env_03_modern_counter_trash_no_avatar.png')));

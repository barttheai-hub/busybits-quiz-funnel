...
router.post('/', (req, res, next) => {
  try {
    const task = {
      ...
      status: oneOf(req.body.status || 'To Do', ['To Do', 'In Progress', 'Done', 'Blocked'], { field: 'status', required: true }),
...
router.put('/:id', (req, res, next) => {
  try {
    ...
    const updated = {
      ...
      ...(req.body.status !== undefined ? { status: oneOf(req.body.status, ['To Do', 'In Progress', 'Done', 'Blocked'], { field: 'status', required: true }) } : {}),
